...
def pzt_scan(pzt_motor, start, stop, steps, detectors=[Vout2], sleep_time=1, md=None):
    ...
    _md = {'detectors': [det.name for det in detector_all],
           'detector_set_read': [det.name for det in detector_set_read],
           'motors': [motor.name],
           'XEng': XEng.position,
           'plan_args': {'pzt_motor': pzt_motor.name,
                         'start': start,
                         'stop': stop,
                         'steps': steps,
                         'detectors': 'detectors',
...
def pzt_scan_multiple(moving_pzt, start, stop, steps, detectors=[Vout2], repeat_num=2, sleep_time=1, fn='/home/xf18id/Documents/FXI_commision/DCM_scan/'):
    '''
...
def pzt_energy_scan(moving_pzt, start, stop, steps, eng_list, detectors=[dcm.th2, Vout2], repeat_num=1,sleep_time=1, fn='/home/xf18id/Documents/FXI_commision/DCM_scan/'):
    '''
    ...
    for eng in eng_list:
        ...
        yield from pzt_scan_multiple(pzt_dcm_th2, start, stop, steps, detectors, repeat_num=repeat_num, sleep_time=sleep_time, fn=fn)
...
def pzt_overnight_scan(moving_pzt, start, stop, steps, detectors=[dcm.th2, Vout2], repeat_num=10, sleep_time=1, night_sleep_time=3600, scan_num=12, fn='/home/xf18id/Documents/FXI_commision/DCM_scan/'):
    ...
    for i in range(scan_num):
        ...
        yield from pzt_scan_multiple(pzt_dcm_th2, start, stop, steps, detectors, repeat_num=repeat_num, sleep_time=sleep_time, fn=save_file_dir)
