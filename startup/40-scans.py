...
def _xanes_per_step(eng, detectors, motor, move_flag=1, move_clens_flag=1, info_flag=0):
    yield from move_zp_ccd(eng, move_flag=move_flag, move_clens_flag=move_clens_flag,info_flag=info_flag)
...
def xanes_scan(eng_list, exposure_time=0.1, chunk_size=5, out_x=0, out_y=0, out_z=0, out_r=0, simu=False, relative_move_flag=1, note='', rot_first_flag=1, md=None):
    '''
    ...
    def xanes_inner_scan():
        ...
        yield from _move_sample_out(motor_x_out, motor_y_out, motor_z_out, motor_r_out, repeat=2, rot_first_flag=rot_first_flag)
        print('\ntake bkg image after xanes scan, {} per each energy...'.format(chunk_size))
        for eng in eng_list:
            yield from _xanes_per_step(eng, detectors, motor, move_flag=1, info_flag=0)
        yield from _move_sample_in(motor_x_ini, motor_y_ini, motor_z_ini, motor_r_ini, repeat=2, trans_first_flag=rot_first_flag)
...
def xanes_scan2(eng_list, exposure_time=0.1, chunk_size=5, out_x=0, out_y=0, out_z=0, out_r=0, simu=False, relative_move_flag=1, note='', flt=[],rot_first_flag=1,  md=None):
    '''
    ...
    def xanes_inner_scan():
        ...
        for eng in eng_list:
            yield from _xanes_per_step(eng, detectors, motor, move_flag=1, move_clens_flag=1, info_flag=0)
            if len(flt):
                for filt in flt:
                    yield from mv(filt, 1) 
                    yield from bps.sleep(0.5)
            yield from _take_bkg_image(motor_x_out, motor_y_out, motor_z_out, motor_r_out, detectors, motor, num_bkg=1, simu=simu, traditional_sequence_flag=rot_first_flag)
            yield from _move_sample_in(motor_x_ini, motor_y_ini, motor_z_ini, motor_r_ini, repeat=2, trans_first_flag=rot_first_flag)
            if len(flt):
                for filt in flt:
...
def xanes_scan3(eng_list, exposure_time=0.1, chunk_size=5, out_x=0, out_y=0, out_z=0, out_r=0, simu=False, relative_move_flag=1, note='', filters=[], rot_first_flag=1, md=None):
    '''
    Different from xanes_scan:  In xanes_scan2, it moves out sample and take background image at each energy
...
def eng_scan_delay(start, stop=None, num=1, detectors=[ic3, ic4], delay_time=1, note='', md=None):
    '''
...
def fly_scan(exposure_time=0.1, relative_rot_angle = 180, period=0.15, chunk_size=20, out_x=None, out_y=2000, out_z=None,  out_r=None, rs=1, note='', simu=False, relative_move_flag=1, rot_first_flag=1, filters=[], rot_back_velo=30, md=None):
    '''
    Inputs:
    -------
    exposure_time: float, in unit of sec
...
#        yield from abs_set(zps.pi_r.velocity, rs)
        for flt in filters:
            ...
            yield from mv(flt, 1)        
        yield from bps.sleep(1)
        yield from _take_bkg_image(motor_x_out, motor_y_out, motor_z_out, motor_r_out, detectors, motor, num_bkg=1, simu=False, traditional_sequence_flag=rot_first_flag)
        yield from _close_shutter(simu=simu)
        yield from _move_sample_in(motor_x_ini, motor_y_ini, motor_z_ini, motor_r_ini, trans_first_flag=rot_first_flag)
        for flt in filters:
            yield from mv(flt, 0)
    uid = yield from fly_inner_scan()
    yield from mv(Andor.cam.image_mode, 1)
...
def raster_2D_scan2(x_range=[-1,1],y_range=[-1,1],exposure_time=0.1, out_x=0, out_y=0, out_z=0, out_r=0, img_sizeX=2560,img_sizeY=2160,pxl=17.2,  num_bkg=1, simu=False, relative_move_flag=1,rot_first_flag=1, note='', scan_x_flag=1, md=None):
    '''
    scanning large area by moving samples at different 2D block position, defined by x_range and y_range, only work for Andor camera at full resolution (2160 x 2560)
    for example, set x_range=[-1,1] and y_range=[-2, 2] will totally take 3 x 5 = 15 images and stitch them together
    ...
    def raster_2D_inner():
        ...
        for ii in np.arange(x_range[0],x_range[1]+1):
            ...
            for jj in np.arange(y_range[0], y_range[1]+1):
                ...
                yield from _take_image(detectors, motor, 1)
#                yield from trigger_and_read(list(detectors) + motor)
                yield from _take_bkg_image(motor_x_out, motor_y_out, motor_z_out, motor_r_out, detectors, motor, num_bkg=num_bkg, simu=simu,traditional_sequence_flag=rot_first_flag)
...
def multipos_2D_xanes_scan(eng_list, x_list, y_list, z_list, r_list, out_x, out_y, out_z, out_r, chunk_size=5, exposure_time=0.1, repeat_num=1, sleep_time=0, relative_move_flag=1, note=''):
    num = len(x_list)
    ...
    for rep in range(repeat_num):
        ...
        for i in range(num):
            ...
            yield from xanes_scan2(eng_list, exposure_time=exposure_time, chunk_size=chunk_size, out_x=out_x, out_y=out_y, out_z=out_z, out_r=out_r, relative_move_flag=relative_move_flag, note=my_note)
        yield from bps.sleep(sleep_time)
...
def raster_2D_xanes3(eng_list, x_range=[-1,1],y_range=[-1,1],exposure_time=0.1, out_x=None, out_y=None, out_z=None, out_r=None, img_sizeX=2560,img_sizeY=2160,pxl=20,  simu=False, relative_move_flag=1,rot_first_flag=1, note='', md=None):

    insert_text('raster_2D_xanes3 contains following raster_2D_scans:')
    for eng in eng_list:
        yield from move_zp_ccd(eng, move_flag=1, info_flag=1, move_clens_flag=1, move_det_flag=0)
        yield from raster_2D_scan(x_range, y_range, exposure_time, out_x, out_y, out_z, out_r, img_sizeX, img_sizeY, pxl,  simu, relative_move_flag, rot_first_flag, note, scan_x_flag=1, md=md)
...
def xanes_3D(eng_list, exposure_time=0.05, relative_rot_angle=185, period=0.05, out_x=0, out_y=0, out_z=0, out_r=0, rs=2, simu=False, relative_move_flag=1, traditional_sequence_flag=1, note=''):
    ...
    for eng in eng_list:        
        ...
        yield from fly_scan(exposure_time, relative_rot_angle=relative_rot_angle, period=period, out_x=out_x, out_y=out_y, out_z=out_z, out_r= out_r, rs=rs, note=my_note, simu=simu, relative_move_flag=relative_move_flag)
        yield from bps.sleep(1)
    yield from mv(Andor.cam.image_mode, 1)    
    export_pdf(1)
...
def fly_scan_repeat(exposure_time=0.03, relative_rot_angle = 185, period=0.05, chunk_size=20, x_list=[], y_list=[], z_list=[], out_x=0, out_y=-100, out_z=0,  out_r=0, rs=6, note='', repeat=1, sleep_time=0, simu=False, relative_move_flag=1, rot_first_flag=1, rot_back_velo=30, md=None):    
    nx = len(x_list) 
    ny = len(y_list)
    nz = len(z_list)
    if nx==0 & ny==0 & nz==0:
        for i in range(repeat):
            yield from fly_scan(exposure_time=exposure_time, relative_rot_angle = relative_rot_angle, period=period, chunk_size=chunk_size, out_x=out_x, out_y=out_y, out_z=out_z,  out_r=out_r, rs=rs, note=note, simu=simu, relative_move_flag=relative_move_flag, rot_first_flag=rot_first_flag, rot_back_velo=rot_back_velo, md=md)    
